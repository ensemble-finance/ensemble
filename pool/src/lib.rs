...
pub mod pallet {
    ...
    use pool_trait::pool::{
        Assets, Bound, ConstantMeanMarket, FixedBalance, PoolConfig, PoolInfo, Reserve, WeightsVec,
    };
    ...
    pub trait Config: frame_system::Config {
        ...
        type Weight: PerThing + FullCodec + Default + Encode + Decode + MaxEncodedLen + TypeInfo;
        ...
        #[pallet::constant]
        type PoolSize: Get<u32> + Debug + TypeInfo + Clone;
    }
    ...
    #[allow(missing_docs)]
    pub type PoolSizeOf<T> = <T as Config>::PoolSize;
    ...
    pub type Pools<T: Config> = StorageMap<_, Twox64Concat, T::PoolId, PoolInfoOf<T>>;
    ...
    where
        ...
        <T as Config>::PoolSize: Ord + Get<u32>,
    {
        // # Errors
        ...
        pub fn deposit(
            ...
            <Self as ConstantMeanMarket>::all_asset_deposit(&from, &pool_id, deposits.clone())?;
            Ok(().into())
        }
        ...
        //          let from = ensure_signed(origin)?;
        //
        //          <Self as ConstantMeanMarket>::single_asset_deposit(&from, &pool_id, deposits.clone())?;
        //          Ok(().into())
        //      }
        ...
        pub fn withdraw(
            ...
            <Self as ConstantMeanMarket>::all_asset_withdraw(&to, &pool_id, lp_amount)?;
            Ok(().into())
        }
    }
        ...
        <T as Config>::PoolSize: Ord + Get<u32>,
    {
        ...
        /// - `PoolSizeIsOutsideOfAssetBounds`: `config.asset_bounds.maximum` < `config.assets.len()`.
        ...
        fn create(
            ...
            // Requirement 2) The number of assets in the pool's configuration must be within
            //     the required upper bound
            let pool_size = number_of_assets as u8;
            ensure!(
                pool_size as u32 <= Self::PoolSize::get(),
                Error::<T>::PoolSizeIsOutsideOfAssetBounds
            );

            // ---------- Weight Requirements ----------
            // Requirement 3) There exists a corresponding weight for each asset
            ensure!(
                Self::each_asset_has_exactly_one_corresponding_weight(
                    &config.assets,
                    &config.weights
                ),
            ...
            // Requirement 4) The weights must be greater than zero
            ensure!(
                Self::weights_are_nonnegative(&config.weights),
                Error::<T>::PoolWeightsMustBeNonnegative
            );

            // Requirement 5) The weights must be normalized (sum to one)
            ensure!(
            ...
            // Requirement 8) The user who issued must have the balance they are trying to deposit
                ...
                Self::user_has_specified_balance_for_asset(
                    &from,
                    T::NativeAssetId::get(),
                    creation_fee.amount
                ),
            ...
            Self::deposit_event(Event::PoolCreated { pool_id, pool_info });
        ...
        fn all_asset_deposit(
            ...
            let reserve_total: BalanceOf<T> = Self::reserves_of(pool_id)?
                .iter()
                .fold(BalanceOf::<T>::zero(), |total, reserve| {
                    total + reserve.amount
                });
            let deposit_total: BalanceOf<T> = deposits
                .iter()
                .fold(BalanceOf::<T>::zero(), |total, reserve| {
                    total + reserve.amount
                });
            ...
            for deposit in &deposits {
                ...
                // Requirement 6) deposit amounts match the pools current reserve ratio
                    ...
                    Self::deposit_matches_underlying_value_distribution(
                        pool_id,
                        deposit,
                        deposit_total,
                        reserve_total
        ...
        fn all_asset_withdraw(
            ...
            let share_total: BalanceOf<T> = lps_share
                .iter()
                .fold(T::Balance::zero(), |total, reserve| total + reserve.amount);
    ...
    impl<T: Config> Pallet<T> {
        ...
        fn do_spot_price(
            ...
            let asset_weight: FixedBalance =
                FixedBalance::from_num(asset_weight.deconstruct().into())
                    .checked_div(FixedBalance::from_num(
            ...
            let numeraire_balance: FixedBalance =
                FixedBalance::saturating_from_num(numeraire_balance);

            let numeraire_weight: WeightOf<T> = PoolAssetWeight::<T>::get(pool_id, numeraire);
            let numeraire_weight: FixedBalance =
                FixedBalance::from_num(numeraire_weight.deconstruct().into())
                    .checked_div(FixedBalance::from_num(
                        WeightOf::<T>::one().deconstruct().into(),
                    ))
                    .ok_or(ArithmeticError::Overflow)?;

            let numerator: FixedBalance = asset_balance
                .checked_div(asset_weight)
                .ok_or(ArithmeticError::Overflow)?;

            let denominator: FixedBalance = numeraire_balance
                .checked_div(numeraire_weight)
                .ok_or(ArithmeticError::Overflow)?;

            let result: FixedBalance = numerator
            ...
            <T as Config>::PoolSize: Ord + Get<u32>,
        {
            PoolCount::<T>::try_mutate(|id| {
                ...
                for asset_id in config.assets.iter() {
                    // TODO (Nevin):
                ...
                for share in config.weights.iter() {
        ...
        fn do_all_asset_deposit(
            ...
            for deposit in &deposits {
                ...
                let _vault_lp_token_amount = T::Vault::deposit(vault_id, to, deposit.amount)
                    .map_err(|_| Error::<T>::DepositingIntoVaultFailed)?;

                // Requirement 3) Update the pool's reserve runtime storage objects
                Self::increase_pool_asset_balance_storage(
                    pool_id,
                    &deposit.asset_id,
                    &deposit.amount,
                )?;
                Self::increase_pool_asset_total_balance_storage(
                    pool_id,
                    &deposit.asset_id,
                    &deposit.amount,
                )?;
            ...
            T::Currency::mint_into(pool_info.unwrap().lp_token, from, lp_tokens_to_mint)
        ...
        fn do_all_asset_withdraw(
            ...
            for asset_share in &lps_share {
                ...
                let vault_balance_withdrawn =
                    T::Vault::withdraw(vault_id, pool_account, asset_share.amount)?;
            ...
            for asset_share in &lps_share {
                PoolAssetTotalBalance::<T>::mutate(
                    ...
                    |balance| -> DispatchResult {
                        *balance = balance
                            .checked_sub(&asset_share.amount)
                            .ok_or(ArithmeticError::Overflow)?;
                        Ok(())
                    },
                )?;
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn reserves_of(
            ...
            let assets =
                PoolAssets::<T>::try_get(pool_id).map_err(|_err| Error::<T>::PoolDoesNotExist)?;
        ...
        pub fn weight_of(
            pool_id: &T::PoolId,
            asset_id: &T::AssetId,
        ) -> Result<WeightOf<T>, DispatchError> {
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn each_asset_has_exactly_one_corresponding_weight(
            assets: &[AssetIdOf<T>],
            weights: &WeightsVec<AssetIdOf<T>, WeightOf<T>, PoolSizeOf<T>>,
        ) -> bool {
            ...
            let assets = BTreeSet::<AssetIdOf<T>>::from_iter(assets.iter().copied());

            // Get unique asset_ids from 'weights'
            let weights =
                BTreeSet::<AssetIdOf<T>>::from_iter(weights.iter().map(|weight| weight.asset_id));
        ...
        pub fn weights_are_nonnegative(
            ...
            weights.iter().all(|weight| zero < weight.weight)
        }
            ...
            weights
                ...
                .all(|weight| match (lower_bound, upper_bound) {
                    ...
                    (Some(minimum), Some(maximum)) => {
                        minimum <= weight.weight && weight.weight <= maximum
                    }
                })
        }
        ...
        pub fn there_is_one_deposit_for_each_underlying_asset(
            ...
            let underlying_assets: Assets<AssetIdOf<T>, PoolSizeOf<T>> =
                PoolAssets::<T>::get(&pool_id).ok_or(Error::<T>::PoolDoesNotExist)?;

            let underlying_assets: BTreeSet<AssetIdOf<T>> =
                BTreeSet::<AssetIdOf<T>>::from_iter(underlying_assets.iter().copied());

            let deposit_assets: BTreeSet<AssetIdOf<T>> = BTreeSet::<AssetIdOf<T>>::from_iter(
                deposits.iter().map(|deposit| deposit.asset_id),
            );

            Ok(underlying_assets.is_subset(&deposit_assets)
                && underlying_assets.is_superset(&deposit_assets))
        }
        ...
        pub fn user_has_specified_balance_for_deposits(
            ...
            deposits.iter().all(|deposit| {
                Self::user_has_specified_balance_for_asset(user, deposit.asset_id, deposit.amount)
            })
        }
        ...
        pub fn deposit_matches_underlying_value_distribution(
            ...
            Ok(lower_bound <= deposit_value_distribution
                && deposit_value_distribution <= upper_bound)
        ...
        fn asset_is_tracked_by_pool(pool_id: &T::PoolId, asset_id: &AssetIdOf<T>) -> bool {
        ...
        pub fn withdraw_is_within_pools_withdraw_bounds(
            ...
            let lp_amount: u128 = <T::Convert as Convert<BalanceOf<T>, u128>>::convert(lp_amount);

            let lp_circulating_supply = Self::lp_circulating_supply(pool_id)?;
            let lp_circulating_supply: u128 =
                <T::Convert as Convert<BalanceOf<T>, u128>>::convert(lp_circulating_supply);

            let lp_share: WeightOf<T> =
                WeightOf::<T>::from_rational(lp_amount, lp_circulating_supply);
    ...
    impl<T: Config> Pallet<T> {
        ...
        fn increase_pool_asset_total_balance_storage(
            ...
            PoolAssetTotalBalance::<T>::mutate(pool_id, asset_id, |balance| -> DispatchResult {
                *balance = balance
                    .checked_add(amount)
                    .ok_or(ArithmeticError::Overflow)?;
                Ok(())
        ...
        pub fn required_creation_deposit_for(
        ...
        fn weighted_geometric_mean(
            ...
            for deposit in deposits {
                ...
                let weight: FixedBalance = FixedBalance::from_num(
                    weight.deconstruct().into() as f64
                        / WeightOf::<T>::one().deconstruct().into() as f64,
                );

                result = result
                    .checked_mul(pow(balance, weight).map_err(|_| ArithmeticError::Overflow)?)
                    .ok_or(ArithmeticError::Overflow)?;
            }

            let result: u128 = FixedBalance::saturating_to_num::<u128>(result);
            let result: BalanceOf<T> = <T::Convert as Convert<u128, BalanceOf<T>>>::convert(result);
        ...
        fn increase_in_weighted_geometric_mean(
            ...
            for deposit in deposits {
                let deposit_amount: u128 =
                    <T::Convert as Convert<BalanceOf<T>, u128>>::convert(deposit.amount);
                let deposit_amount: FixedBalance =
                    FixedBalance::saturating_from_num(deposit_amount);
                ...
                let asset_ratio: FixedBalance = weight
                    .checked_mul(deposit_amount)
                    .ok_or(ArithmeticError::Overflow)?
                    .checked_div(reserve)
                    .ok_or(ArithmeticError::Overflow)?;

                deposit_ratio = deposit_ratio
                    .checked_add(asset_ratio)
                    .ok_or(ArithmeticError::Overflow)?;
            }
            ...
            let lp_circulating_supply: u128 =
                <T::Convert as Convert<BalanceOf<T>, u128>>::convert(lp_circulating_supply);
            let lp_circulating_supply: FixedBalance =
                FixedBalance::saturating_from_num(lp_circulating_supply);

            // Converts the floor of the FixedBalance into a u128
            let lp_tokens_to_mint: FixedBalance = lp_circulating_supply
                .checked_mul(deposit_ratio)
                .ok_or(ArithmeticError::Overflow)?
                .ceil();
            let lp_tokens_to_mint: u128 =
                FixedBalance::saturating_to_num::<u128>(lp_tokens_to_mint);
        ...
        fn lps_share_of_pool(
            ...
            for asset_id in assets.iter() {
                ...
                let lp_share_of_asset: T::Balance =
                    Self::lps_share_of_asset(reserve, lp_amount, lp_circulating_supply)
                        .map_err(|_| ArithmeticError::Overflow)?;

                lps_share.push(Reserve {
                    asset_id: *asset_id,
        ...
        fn lps_share_of_asset(
            ...
            let lp_amount: u128 = <T::Convert as Convert<T::Balance, u128>>::convert(lp_amount);
